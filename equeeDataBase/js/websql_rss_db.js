...
function getFeed(){
		    ...
		    $(xml).find("item").each(function () {
		    	  var url =  $(this).find("enclosure").attr('url')
			        ...
			        $("#rssContent").append('<div class="feed"><div class="image"><img src=' + url + ' width=' + width + 'px /><div class="title"> Title:' + $(this).find("title").text() 
			        		+ '</div><br><div class="web-description">Desc: ' + $(this).find("description").text() + '</div>'+'</div><br><div class="time">'+$(this).find("pubDate").text().substr(17,8)+' '+$(this).find("pubDate").text().substr(5,11)+'</div>');
 ...
 tx.executeSql('CREATE TABLE IF NOT EXISTS ' + storeName + ' (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, description TEXT, image TEXT)', [],
  ...
  tx.executeSql('INSERT INTO ' + storeName + ' (title, description, image , time) VALUES (?, ?, ?)', [title_, description_, image_, time_], null, onError);
         });
         };
         ...
         db.transaction(function (tx) {
            tx.executeSql('SELECT * FROM ' + storeName, [], function (tx, results) {
               ...
               for (i = 0; i < len; i++){
                  msg = "<div><p><b>" + results.rows.item(i).title + "</b></p><p>"+results.rows.item(i).description+"</p></div>";
                  $("#rssContent").append(msg);
               }
            }, onError);
         });
